...
function transformDatabaseProject(dbProject: DatabaseProject): Project {
  return {
    ...
    responsible: dbProject.responsible as any,
...
function transformToDatabase(project: Partial<Project>): Partial<DatabaseProject> {
  return {
    ...
    ...(project.estimatedEndDate && { estimated_end_date: project.estimatedEndDate }),
...
function calculateProgressFromTrello(card: any): number {
  if (card.badges?.checkItems && card.badges?.checkItemsChecked) {
    return Math.round((card.badges.checkItemsChecked / card.badges.checkItems) * 100);
  }
